// Copyright (c) 2018 Sophie Kirschner
// https://github.com/pineapplemachine/interval-tree-type-js/blob/master/LICENSE
...
class IntervalTree {
    ...
    while (true) {
      if (low < node.low) {
        if (node.left) {
          ...
          // Add the interval to a new left child of this node
        ...
        if (node.right) {
          ...
          // Add the interval to a new right child of this node
  ...
  *queryPoint(point) {
    ...
    while (stack.length) {
      ...
      if (
        node.left &&
        point <= node.left.maximumHigh &&
        point >= node.left.minimumLow
      ) {
        stack.push(node.left);
      }
      if (
        node.right &&
        point <= node.right.maximumHigh &&
        point >= node.right.minimumLow
      ) {
  ...
  *queryInterval(low, high) {
    ...
    while (stack.length) {
      ...
      if (
        node.left &&
        high >= node.left.minimumLow &&
        low <= node.left.maximumHigh
      ) {
        stack.push(node.left);
      }
      if (
        node.right &&
        high >= node.right.minimumLow &&
        low <= node.right.maximumHigh
      ) {
        stack.push(node.right);
      }
    }
  ...
  *queryWithinInterval(low, high) {
    ...
    while (stack.length) {
      ...
      if (
        node.left &&
        node.left.maximumLow >= low &&
        node.left.minimumHigh <= high
      ) {
        stack.push(node.left);
      }
      if (
        node.right &&
        node.right.maximumLow >= low &&
        node.right.minimumHigh <= high
      ) {
        stack.push(node.right);
      }
    }
...
class IntervalTreeNode {
  ...
  addLeftChild(tree, low, high, value) {
    this.left = new IntervalTreeNode(this.valuesEqual, low, high, this, Red);
  ...
  addRightChild(tree, low, high, value) {
    this.right = new IntervalTreeNode(this.valuesEqual, low, high, this, Red);
